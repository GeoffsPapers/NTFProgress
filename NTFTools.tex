...
\item {\tt \$modalities == \$modal\_system\_M} - This modality is built from:
      \begin{packed_itemize}
      \item The necessitation rule of {\bf K}: If $A$ is the theorem of {\bf K} then so is $\Box A$
      \item The distribution axiom of {\bf K}: 
            $\Box (A \rightarrow B) \rightarrow (\Box A \rightarrow \Box B)$
      \item The {\bf D} axiom: $\Box A \rightarrow \Diamond A$, i.e., the accessibility relation is
            serial.
...
elements that exist in {\tt w1}, i.e., the subformulae requires that the domain element 
